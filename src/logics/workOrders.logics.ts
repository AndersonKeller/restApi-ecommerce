...
import { IWorkOrderFinish, IWorkOrderPartsRequest, IWorkOrderRequest, WorkOrderMechanicalResult, WorkOrderPartsResult, WorkOrderResult } from '../interfaces/workOrders.interfaces'
import { client } from '../database'
import { QueryConfig } from 'pg'

const createWorkOrders = async (req: Request, res: Response): Promise<Response> => {
...
const listWorkOrders = async (req: Request, res: Response): Promise<Response> => {
    ...
    const queryResult: WorkOrderMechanicalResult = await client.query(queryString)

    return res.json(queryResult.rows)
}

const finishWorkOrders = async (req: Request, res: Response) => {
...
const createPartsInWorkOrder = async (req: Request, res: Response): Promise<Response> => {
    ...
    queryConfig = {
        text: queryString,
        values: [workOrderId, queryResultParts.rows[0].id, workOrderPartsData.quantity]
    }

    const queryResultWorkOrderParts: WorkOrderPartsResult = await client.query(queryConfig)
    ...
    queryConfig = {
        text: queryString,
        values: [queryResultParts.rows[0].quantityInStock - workOrderPartsData.quantity, queryResultParts.rows[0].id]
    }
...
const listWorkOrderParts = async (req: Request, res: Response): Promise<Response> => {
